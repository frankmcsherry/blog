...
A slightly lazy worker `D` who doesn't receive any messages can just receive the three progress messages and confirm that they cancel out and that it shouldn't expect anything more. Workers `A`, `B`, and `C` don't wait on `D`, who can be asleep if it really wants (or maybe working on something else). Nor do the worker explicitly syncronize; they can't fire notifications until they learn that work is done, but they can work on whatever else they like in the meantime.
