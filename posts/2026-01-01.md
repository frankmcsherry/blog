...
It's very powerful, and in my opinion **essential** for declarative recursive programming, but it's often ignored.
...
Declarative programs tell the computer what needs to be true of the output, but leave it up to the computer to actually find the answer, which has the potential to result in a surprising path to arrive at the output.
...
If you have many parent relationships, for example for the 8.2B people currently alive, as well as lots of historical data, you could imagine `Ancestor` growing substantially as we go back in time.
Only some 100B have ever lived, so your family tree eventually folds in on itself, but even your ancestors by themselves are really quite a lot of data, to say nothing of the 8B other people like you.
...
Perhaps fine for a simple concept like ancestry, but less fine for more complex rules like the Rust borrow checker.
...
The behavior left a fair bit to be desired, but my understanding.
...
The rules are relatively concise, if not simple, and start from seven external tables (EDBs in Datalog-ese).
```
 facts input name
 ----- -----
  1886 borrow_region
...
```
The output at the end that we are interested in is `errors`, though .. heart of the problem here .. that isn't evident from the Datalog itself.
...
Unfortunately `datafrog` is **imperative**.
...
This is where we introduce the **demand transform**.

## The Demand Transform

The demand transform rewrites a Datalog program around the "seed crystal" that is your query, and follows rules backwards, from heads to badies, searching for derivations that might produce something relevant for the query.
...
Let's take a moment and see where these facts could possibly come from.

The first rule will produce relevant facts only when `Y` is `'Frank'`, and we could rewrite that rule as
```prolog
Ancestor(X, 'Frank') :- Parent(X, 'Frank').
```
The second rule will produce relevant facts only when `Z` is `'Frank'`, and we could rewrite that rule as
```prolog
...
```
The second rule .. is now weirder.
It will also produce relevant facts from `Parent('Confucius', Y)`, but to do so it .. may need all of `Ancestor`?
We don't actually know *a priori* how to restrict `Ancestor` to produce the right results, because `'Confucius'` doesn't appear in any of the facts that we need.

The observation, the first step on the path to the demand transform, is that not only will we need ("demand") ancestry from Confucius, but also from Confucius's children, and their children, and so on.
We aren't going to be able to directly derive only `Ancestor('Confucius', Y)`, but we may be able to narrow the set of literals in that first position.

Informally, let's introduce a new variable `Ancestor_dX(X)`, which is meant to indicate the `X` values we'll need to see for our query.
Initially there is only one rule that populates this variable:
```prolog
Ancestor_dX('Confucius') :- .
```
...
```prolog
Ancestor_dX(Y) :- Ancestor_dX(X), Parent(X, Y).
```
...
The `Ancestor_dX` relation is used to "gate" the derivations of `Ancestor`.
Before we produce a fact we need to make sure it is demanded, and a way to do this is to add `Ancestor_dX` as a clause to each `Ancestor` rule:
```prolog
Ancestor(X, Y) :- Ancestor_dX(X), Parent(X, Y).
Ancestor(X, Z) :- Ancestor_dX(X), Parent(X, Y), Ancestor(Y, Z).
```
If we combine this with our two rules to derive `Ancestor_dX` in the first place, we end up with a derivation of the descendants of Confucius.
It's likely that with some smarter thinking, we could further optimize `Ancestor` away entirely, and just have `Ancestor_dX` as `SmartDescendant`.
...
1. Your query itself produces a demand pattern, for the named relation and with a sequence with "bound" where you use literals.
2. For any rule whose head has a demand pattern `(Foo, s)`, add a demand pattern for each `bi` in the rule body, where its sequence has "bound" in a position if any of;
    ...
    3.  That position has a variable that appears in the head, and is "bound" in `s`.
...
Let's take a moment and look at our collection of demand patterns.
```
(errors, 'ff')
(invalidates, 'ff')
(borrow_live_at, 'bb')
...
3.  The third rule produces the pattern `(subset, 'bbb')`.
    ...
    My sense is that it should always be legal to weaken a constraint.
...
In the paper their names are all prefixed with a `d_` which breaks the Prolog styling, atoms start with capital letters, but the whole Polonius logic also goes against that so .. argh.
...
```
The first of these two is clearly a bit silly, so we'll not keep it.
...
We'll keep various rules for head atmos we are not "demand transforming", and replace the rules whose heads are demand transformed.
