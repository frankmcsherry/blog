...
By the end of the post I hope to have explained the transform to you, motivated its application even in batch data processing, and optimize a Datalog formulation of the Rust borrow checker almost to zero (update: this is not news to the Rust folks).
...
Folks on the Rust team explained that they are aware of this sort of work, and in particular the ability for the demand transform to improve the naive analysis.
They have a "hybrid" variant that will pick between easy and harder rules based on the "potential errors", also determined using Datalog (through my reading).
The hybrid time too about 14ms when I ran it locally, so it checks out that it is fairly similar to the demand-transformed forms of the rules.

We are in touch about whether this fizzles as things get complicated, or whether it just gets harder to do manually.
I imagine I'll learn more, and if it turns out amazing I'll share it!

Thanks very much to the Rust folks for their openness, and eagerness to collaborate!
