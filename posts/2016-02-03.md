...
I can't really tell what their concern is here. My best guess is that they think that by random subsampling Lithuanian women, privacy is somehow well protected and you shouldn't have to add any more random noise.

Anyhow, differential privacy working as intended. Clueless people don't get clear access to sensitive data about small populations, no matter how safe they think it should be.
...
That's the text from the email I send them those years back. The authors know this stuff exists, they just don't want to tell you about it. Instead, they write 
...
To be fair to the authors, the next section is about "hey what if you count things instead", where they use tables rather than cumulative density functions. Maybe because they really wanted to avoid illustrations.
...
This is a bit disappointing, because the cumulative density function doesn't have "crude categories", it just shows you the continuous distribution. Ok, the lack of forthrightness is getting pretty standard now.
...
Yes, well a stupid researcher would. However, if this researcher has taken an elementary class in probability and statistics, they understand that noisy measurements reflect information about the true data, but are not actually the true data. If you were to phone up 1,000 random people in New York and ask for their salaries, you might take the average you hear as a reasonable representative. On the other hand, you should not conclude that only 1,000 people in New York are employed, and so the average is quite low. You need to pay attention to how you got the data, not just slot it in to your excel spreadsheet.

Fortunately, [Bayes' rule](https://en.wikipedia.org/wiki/Bayes%27_theorem) got invented a long time ago, and tells us exactly how to do this. For each measurement, you use the probability of seeing the measurement given some true data, combined with your prior belief about the true data, to produce a new "posterior" belief about the data. Your prior belief about the number of billionaires in the population may be "99.999999% none", and seeing the number 5 updates this to "99.999998% none". Concluding "golly gee; there must be five" takes a special sort of education.
...
This is a horrible way to compute such an average. You went and added the noise you would add to large sums over many people to a single [-1.0, 1.0] number. 
...
Do not first sum and divide, and then add noise.
...
2. Laplace noise addition adds way too much noise to statistics that can fluctuate wildly with few users.
   ...
   Also true. But for exactly those fluctuation reasons, statisticians use better statistical estimators. Differential privacy seems rather compatible with the improved estimators. You are welcome!

3. People may think that noised counts are true, rather than noisy.
...
The section is worth a read for their takedowns of quite famous and well-regarded folks, using their findings from the previous sections as ammunition. I just don't see how it ends well for the authors.
...
Perhaps, independent of your ultimate take on differential privacy, the legal and policy community could curb its enthusiasm for incompetent and/or intentionally dishonest research articles. 