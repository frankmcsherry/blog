...
As we run our system time capability forward, repeatedly downgrading to `(system, 0)`, we inform differential dataflow that it is able to discard the distinctions between prior system times. This allows it to compact its representation of the differences, and informally this results in something like a "maintained" trace rather than full historical detail.
...
By moving our capabilities forward, we allow differential dataflow to compact historical differences, and if `system` and `event` are pretty much the same then we behave almost as if a uni-temporal stream processor.
