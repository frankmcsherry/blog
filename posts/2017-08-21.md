...
Our plan is to build a binary operator, taking both `edge_stream` and `rank_stream` as input. I'm going to beat down the skeleton of such an operator now, using one of timely dataflow's generic binary operators, `binary_frontier`.
