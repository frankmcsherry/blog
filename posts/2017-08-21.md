...
The edge changes flowing around will have type `((usize, usize), i64)` and the rank changes will have type `(usize, i64)`. The first `usize` field in each of these indicates the source node. The `i64` in each indicate the amount of change. We are going to want to shuffle these changes between workers so that all changes for each node arrive at a single worker.

To shuffle data between workers we could use the `exchange` operator. It doesn't change the logical contents of the stream, but it does move the records between workers based on a supplied function (from record to integer; we would just pick out the first field). Another way to exchange data is, as part of building a new operator we can specify how its inputs must be shuffle for the operator to be correct. We are going to do that instead.

Our plan is to build a binary operator, taking both `edge_stream` and `rank_stream` as input. Because we may be running with multiple workers, we'll want to shuffle each stream so that changes to the same source arrive at the same worker. I'm going to blat down the skeleton of such an operator now, using one of timely dataflow's generic binary operators, `binary_frontier`.
...
The `_stash` hash maps are going to be for storing received inputs, respectively edge changes and rank changes, until it is time to process them. This is because we could receive such changes out of order, or one iteration could bleed into the next, that sort of thing. We will stash all changes first, keyed by timestamp, and then extract the changes when it is their round.
...
Having stashed whatever is read on each input, our operator then checks out each of the stashes and asks if there are any stashed times that neither input could now produce. This involves looking at the `frontier()` of each input, which describes which times we should still expect to see. If neither frontier could result in some stashed time, we should process it!
...
```rust
    ...
    for ((src, dst), diff) in edge_changes.drain(..) {
        ...
        // 0. ensure enough state allocated, init 1M surfers.
