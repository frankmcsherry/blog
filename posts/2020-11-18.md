...
![Dataflow graph for join in Materialize]()
...
For example, if we re-run our diagnostic query, the one that counts the records maintained by dataflow, we see
...
If we re-build the query dataflow, and re-pull the statistics on records maintained, we see
