...
3. Differential dataflow enables interactive Datalog computations, both in the sense that 
    1. the source relations can changed arbitrarily, and 
...
Of course, by now all of you are probably familiar with differential dataflow, but let's review some key concepts for folks who may have just gotten here. Differential dataflow does not seem to have a wikipedia page, so I am going to have to make all of this up. 
...
```rust
parents.join_map(parents, |P,&X,&Y| (X,Y))
       .iterate(|same_gen| {
...
I don't have an external database, and I haven't implemented anything like an antijoin yet (`cogroup` would probably work, but I'm lazy). So, I thought I'd take this opportunity to show differential doing something that I think is much cooler, especially as relates to magic sets and querying.
...
There is an identifier `$ID` baked into the query, in the same way that it is baked into the reachability and bfs queries, and you can see that for populating the `pymk` relation we only really care to evaluate `cnt(_, $ID, _)`. But, if you are doing bottom-up semi-naïve Datalog evaluation, and I suspect these folks are, you are probably going to evaluate pretty much all of the `cnt` relation. You can see that they sure take a while to do the computation in the numbers up above.
...
So, although it takes about 10s to load up the data, we stab out and get all of the friends-of-friends of our target `$ID` people in times ranging from 73ms down to 49us. That's pretty cool. It's also a lot faster than doing a fresh computation with `$ID` baked into it for each distinct `$ID`.
...
It takes a little longer to start up, because the graph is a little larger, and now the times range from 32ms down to 255us. Pretty neat, huh?
...
It is worth mentioning that all of the differential dataflow computations above work when the underlying relations change too. I can't be bothered to show you all the times, but for small changes they are all sub-second updates, and often sub-millisecond. It is a lot like the times we saw for making queries, but you are updating the graphs instead of the things that go and meet the graphs. Whatever, it's fast.
...
On our data, the fourth rule in there is the real killer, and it is cyclic. If you do the binary join `p(X,Y), p(X,Z)` first, you produce about two billion tuples, most of which get thrown away when they get checked against `c(Y,Z,W)`. There are better binary join orderings to use, but the Ngo et al algorithm gets to choose between them on *a tuple-by-tuple basis*, and ends up about 3x faster than the best fixed plan for the second rule.
