## Differential privacy for non-experts
...
So let's try and explain differential privacy for non-experts, somewhat clearly. Even for experts there are a few important points in here, which I think some of you have missed. We'll also work through an example where we see one way to evaluate the correlation coefficient of two datasets, which I know some authors are eager to see.

### Differential privacy

There are a large number of definitions of privacy out there. Try and write down what privacy means to you, and you start to understand that it might be a little subtle to really nail down. Some of these definitions are philosophical, for example Warren and Brandeis ("the right to be let alone") or Tore Dalenius ("nothing new should be learned about you when one analyzes your data"). Some are more technical, and have markedly less concise quotes describing their intent. Each definition attempts to describe and motivate a certain flavor of privacy, and while this is an excellent scholarly pursuit, it doesn't immediately translate into helpful advice for the concerned data subject.
...
Differential privacy guarantees that nothing that could not happen without access to your data will happen with access to your data. In fact it makes a stronger quantitative guarantee: The chance that *any specific thing* happens (really, anything at all) with access to your data is at most a multiple X of the chance it would happen without your data. That "multiple" X is part of the guarantee and it determines how much privacy you get: small values like 1.01 are pretty great, whereas values like 10.0 are less amazing but still non-trivial.
...
### Differential privacy protects you from non-experts
...
If the results are published, just the totals with no identifying information, and the number of Islamists in your town is greater than zero, you can bet that as the brownest person around you are going to get beat on by some good ol' boys right quick. They don't have a fancy education, they didn't re-link the published data with auxiliary side information, they saw some number they didn't like and figured they should do something about it. So, privacy accomplished? Which privacy philosopher do you have to thank for your new condition?
...
There is only one thing that doesn't have an "any" in front of it, but it is a bit hidden in the definition. A randomized computation gets provided with random bits as input (imagine a random number generator) to help it make random choices. The computation *is* able to produce different outputs for different input data and bits. However, the number of different input bits for which some output gets produced cannot change too much. That number of bits is what determines the probability the randomized computation produces the output.
...
![https://github.com/frankmcsherry/blog/blob/master/assets/privacy/primer/cdf.png](cumulative density function)

As expected, we see a sharp spike up at around 40%, which is good because we really don't expect to see values far away from that value.
...
![https://github.com/frankmcsherry/blog/blob/master/assets/privacy/primer/cdf2.png](cumulative density function)
...
![https://github.com/frankmcsherry/blog/blob/master/assets/privacy/primer/cdf3.png](cumulative density function)

So, the error we are introducing for 0.1-differential privacy isn't actually worse than the error inherent in this particular measurement process. In this case, it seems totally workable (at least, to me). The concentration is tighter near the correct answer, and a bit looser out in the tails (which should make sense with folks familiar with the Laplace and Gaussian distributions).
...
![https://github.com/frankmcsherry/blog/blob/master/assets/privacy/primer/cdf4.png](cumulative density function)

Because I am brave, and writing the text before doing experiments, I will say that while the constrast isn't as stark as with our counts up above, the error from differential privacy is not drastically worse than the inherent randomness of the standard deviation statistic. TODO: UPDATE; APPEND-ONLY.
...
Let's look at some data for the Pearson correlation coefficient. Perhaps we map our A and B people into the values -1 and 1, respectively, and want to know if there is a correlation between being A or B and being predisposed to chocolate.
...
![https://github.com/frankmcsherry/blog/blob/master/assets/privacy/primer/cdf5.png](cumulative density function)

If instead A people always like chocolate (pairs (-1, 1)) and B people never like chocolate (pairs (1, -1)), we see should something else: a strong negative correlation.

![https://github.com/frankmcsherry/blog/blob/master/assets/privacy/primer/cdf6.png](cumulative density function)
...
![https://github.com/frankmcsherry/blog/blob/master/assets/privacy/primer/cdf7.png](cumulative density function)
...
![https://github.com/frankmcsherry/blog/blob/master/assets/privacy/primer/cdf8.png](cumulative density function)
...
![https://github.com/frankmcsherry/blog/blob/master/assets/privacy/primer/cdf9.png](cumulative density function)

That is getting a little better. How about intervals of size 1

![https://github.com/frankmcsherry/blog/blob/master/assets/privacy/primer/cdf10.png](cumulative density function)
...
1.  You must identify a possible set of output values. 
    ...
    In this case, lets use "32 bit integers" to represent the range from -1 to 1 (so we are mentally dividing our numbers by 2^32).
...
![https://github.com/frankmcsherry/blog/blob/master/assets/privacy/primer/cdf11.png](cumulative density function)

It looks like we have most of our samples around the true median, but it is a bit hard to see the detail. Let's zoom in.

![https://github.com/frankmcsherry/blog/blob/master/assets/privacy/primer/cdf12.png](cumulative density function)

Wow. Quite concentrated.

#### Moar concentration

The concentration above is actually about as *un*-concentrated as it gets. Let's look at another example, where the points are all exactly one value (maybe 0.4). In this case, every output other than exactly 0.4 splits the data very badly, and incurs a penalty of `-|data|`. If we have 1,000 samples, and that 1,000 goes up in an exponent, we could imagine something being very unlikely. Let's count it out:
...
There is a neat generalization of what we did above to a larger class of statistics. It doesn't always work as easily; median happened to be easy to compute, and computing things can be harder than defining them. That being said, for any statistic you have in mind, you can define:
...
For example, you can do "average of -1 to 1 value", like we did up above with Laplace noise, but here get much tighter concentration around the limits: if the value is current -1 (all points are themselves -1) then you have to change half the points to make zero possible. And you have to change all the points to move the average to 1. These would be exponentially unlikely as in the median example above.

It is possible, though I don't know the answer, that you could apply this to the problem of the Pearson correlation coefficient that we studied above. It has a valid range of -1 to 1, and if you impose a bound on the magnitude of the records, you also get concentrated results that are especially concentrated at the limits of its valid range. 

The hard part, unlike median and average, is determining how few records you need to change to reach a give value of the output. Is it piecewise linear and monotonically increasing as you move away from the true value? Can the break-points be easily determined? I don't know the answers to these, but they seem like good questions to ponder! 
