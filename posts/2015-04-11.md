...
In the example above, we might prefer to join the three relations by first joining `R2` and `R3`, which would produce only two records before joining with `R1`.
...
That `flatten()` method simply converts a `Stream<G, (P, Vec<E>)>` to a `Stream<G, (P, E)>`. It is just 17 lines of code somewhere, so hooray for timely dataflow being easy to use (succinct, at least).
