...
* Part 2/x: A very naïve implementation based on sorted lists.
...
Just to start things out, and before trying to be clever at all, let's write an implementation of our trait using Rust's built in `HashMap`. This implementation will be pretty simple, but wasteful with respect to memory usage, and misses out on some important properties we'll want for later on. But, it is a simple example of perhaps the most obvious implementation.
...
The problem is we don't have a `&'a K` anywhere. That `key` argument is a reference, but not with the right lifetime (remember the cautionary tale from the previous section?). The key obviously exists in the `HashMap`, we just can't get a reference to it, because the hash map's interface is a bit restrictive. There is a way to get a reference to a key once you've used the `entry` method up above, but that requires an owned key to call. We can't stash a second copy of the key in the list with the `(V, T, isize)` tuples because we can't copy or clone it.
...
because we know that folks must have a `&K` on hand to call the `get` method. That isn't a horrible idea, but it is limiting; it means that if the user wants the key they either (i) need to use the iterator before moving on to the next key, or (ii) need to clone the key to hold on to it. This is annoying because we know there is a `&'a K` sitting in the hash map, it's just that the hashmap doesn't let us get a handle to it.
...
The `OutputIterator` type isn't going to be wildly complicated. In fact, we could possibly borrow some existing iterator implementations, but we will just roll our own to see how it works. It isn't very complicated. The first thing to do is to define the struct that holds the data we will need for iteration.
...
The `data` field here is, optionally, a triple of key reference, the vector we found, and a `usize` for remembering where we are in the vector. As we advance through the iterator we will increment this `usize`, and stop when it reaches the length of the vector. 

The field is an `Option` type, which can either be `Some(x)` for some data `x`, or `None` indicating that there is nothing there. The option is important here because we do need to return something even when the key is absent and no `&'a K` even exists. We could change our `get` trait to return an `Option<Self::OutputIterator>`, or we can just make our iterator return nothing.
...
The `if let` construction is a neat thing Rust borrowed from Swift (perhaps "copied" would me more accurate, or "cloned" depending on your views on whether ideas have owners). The construct fires if the pattern matches, in this cas if `self.data` is in fact something. At the same time, it is binding `key`, `vec`, and `pos` to references to these fields. In the case of `pos` it is even a mutable reference. The `ref` keyword might be a bit confusing, but I guess the Rust folks thought it was better than `*`, which is logically what you want there (bind `key`, `val`, `pos` to things that derefence to the fields).

This is really all it takes to implement an iterator. 

If you use this type in your code, perhaps by calling `get` on our `HashMap` implementation of `Trace`, Rust will go track down this code and insert it anywhere you call `next`. For example, imagine your value and time types are stack-allocated, like maybe `u64`s or something, and you write the following code:
...
One last thing: to use `K` as a key in a `HashMap` it needs to implement `::std::hash::Hash`, the trait for hashing. So, we just `+` that into its constraints and we are good!
