...
When we build Naiad many years back, while we got our system up and running surprisingly quickly (I was surprised, anyhow), we then spent forever-and-a-half optimizing the performance, and that was almost all about changing our system and programs so that the garbage collector wouldn't have a fit. For example, if you used `String` anywhere in your program, meaning we maintain a large indexed collection of `String`s for you, the C# garbage collector would vanish for seconds at a time (once: 39 seconds) trying to recall just which of the gigabytes of allocations were still live. 
...
If you are familiar with smart pointers in C++, it's roughly the same thing going on here. All bindings (of data to a variable or field) are uniquely owned, in the sense that whenever that binding goes away (e.g. when overwritten, when the containing scope ends, etc) we know that there are no other owners of the data, and we can (and should!) clean up it and any of its resources. Like, right away! We don't have to wait to scan the heap or whatever it is the tracing garbage collectors do.

Consider our `log` type up above. When we construct a new one (which is what the `new()` method does), we are going to allocate up some space for the ten strings it holds. We'll use it for a while, and we might even mutate some of the strinsg, grow them a bit that sort of thing. But, at some point we use it for the last time, and subsequently discard our handle to it. At this moment we can safely go in and de-allocate whatever is pointed at by those ten pointers that were initially allocated for our strings.

There are a bunch of issues with this approach, most prominetnly that if we want a copy of the data for some reason, maybe to hand to another bit of code without giving up our copy, we would need to clone the data. That means another ten new allocations, assuming we stick to the discipline that whenever you discard a `Log` you should de-allocate what memory it owns.
...
Serialization is a lot like user-mode regions. The serialized data live in one allocation and can be created, cloned, and dropped with no more than one allocation (and often fewer, if we plan to serialize a batch of data).
...
Serializing a `Log` to existing memory is about five times faster than allocating a new one. And most of this time is actually reading out of the `Log`; if you want to clone a serialized `Log` to another allocation and then deserialize it, it takes about 23ns.

How could this possibly be? Don't we need to end up with the same thing that `new()` gives us, and if so how could bypassing the language's prefered mechanisms possibly be better? The answer lies in borrowing, and our implicit (now explicit) ability to work only with borrowed data.

Restricting ourselves to work only with borrowed data imposes limitations. We essentially get read-only access to the data, which means we can't take our `Log` and rewrite the `hostname` field. And if we want an owned copy of the `hostname` field to stash somewhere else we will need to clone it. But, the system wasn't planning on handing out ownership of record fields anyhow; we were going to have to clone things.

Let's imagine for now that we are comfortable with using borrowed data, or at least defering any allocations until we actually need owned data. What does a serialization library like [Abomonation](https://github.com/frankmcsherry/abomonation) do under the hood? Let's look at the `encode` and `decode` method signatures first:
...
These two methods are foremost `unsafe`. They muck around with internals that Rust doesn't normally want you touching. They may have undefined behavior, because the Rust folks are still working out what constitutes undefined behavior (at the moment it includes `memcpy` of structs with padding bytes, which Abomonation does).

From its signature, `encode` takes a reference to some type `T` as well as a writeable reference to a vector of bytes. These are the source data and destination vector, respectively. Once done, `bytes` will be extended by a bunch of bytes representing `typed`.

From its signature, `decode` is a complicated beast. Its only argument is `bytes`, a writeable reference to a slice of bytes. This means we can change the bytes themselves, but we can't change the underlying allocation itself. There is an additional input in the type parameter `T` which tells us what type we are hoping to find, without suppling an instance of `T` as an argument. Instead, what we get as output is either `None` (if decoding failed) or `Some` wrapped around a pair of reference to a `T` (the `&T`) and whatever bytes remain having burned some on the decoding.

Both `encode` and `decode` do the thing you would hope they do: respectively they copy the relevant bytes in, and then fix the pointers up internally so you can just pretend the bytes are actually a `T`. It is essentially a `memcpy` on the in-memory representation of a `T`, chasing down pointers to known types like `String` and `Vec` and `Box`. Some types are problematic (e.g. `Rc`) and we just don't implement the `Abomonation` trait for these types.

So, regions. Fast serialization is roughly equivalent to user-mode regions allocation, and spares us a great deal of overhead that comes with using the default allocation strategies. We feel a bit sad that we weren't allocating directly into the region, and we will need to through how we fall back to owned data when we need to inter-op with code that expects it (worst case: do the allocations we just skipped, but only as needed).

But let's look a bit more at serialization, and Abomonation specifically, to see what sorts of things we can easily do.

### Serialization

This is partly me kicking the tires, and partly talking through the sorts of things you should be able to do with serialized data. I'm writing this without having run most of the benchmarks, so it may be that not everything turns out to be a great idea.
...
The first benchmark repeatedly allocates new log entries.
...
This code repeatedly creates a new `Log` entry. It also repeatedly de-allocates the log entry, which is a cost someone is going to have to pay eventually. 

    test log_new           ... bench:         371 ns/iter (+/- 39)

There are ten strings in each `Log`, which means ten allocations and de-allocations. There are also going to be ten-ish calls to `memcpy` to load up the contents of the string, and probably another for the struct itself (which is 288 bytes, even without the allocations).
...
This chats with the allocator to get a new `Vec<u8>`, but it only does so once, and for the appropriate amount of memory for a full encoded `Log`. Additionally, when it copies memory it does so once, rather than eleven distinct times from disjoint regions.

#### Inspecting the data

One sweet advantage of region allocation is *locality*: all of the data we need are co-located, and we don't need to chase down various pointers to distant regions of memory. Let's check out how that looks.

First, what if we just want to look at our decoded data and confirm that it equals what we started from?
