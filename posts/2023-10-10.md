...
Systems can trivially respond with inconsistent or stale results ("nothing yet, boss"), but we understand that this isn't yet helpful.
...
A responsive system needs to promptly provide a *meaningful* response; otherwise it is just entertainment.
...
When `SELECT` commands arrive, from easy `LIMIT 1`s to hard multi-way `JOIN`s, we want to minimize the time we need to take before we can provide the result.
When users create indexes, materialized views, and sinks, we want to minimize the time before those components are operational.
In each case, we want to identify and exploit structure in both the data and the commands to make subsequent work fast.
...
Materialize's storage plane serves up CDC snapshots and changelogs, but in the context of commands that allow it to optimize the data accessed.
...
They also enable `JOIN`s that equate the key columns to run immediately, rather than needing to rescan and reorganize the input.
These indexes are continually maintained, and provide interactive access without sacrificing freshness or consistency as might an independent cache.

Finally, Materialize's serving plane takes advantage of independence among the SQL commands themselves. 
While Materialize must put the commands in *some* order, it can see which commands can execute concurrently and does so.
Materialize tracks the available timestamps for each input and derived view (their "freshness"), and uses this information to avoid sequencing work that cannot yet complete ahead of work that can. 
...
Operational systems need to be ready with the information you need, and be poised to implement the operational work you require.
