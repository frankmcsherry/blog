...
This wll be partly walking through some of the moving parts in differential dataflow, so that you can see them and see how they work, and also partly me having to admit which things are gross and hard to do, so that I can be shamed in to fixing them.

Here is the outline:

1. A little language (borrowed from [Graspan](https://www.ics.uci.edu/~guoqingx/papers/wang-asplos17.pdf)).
...
Let's start by describing a small query language, which comes courtesy of the [Graspan paper](https://www.ics.uci.edu/~guoqingx/papers/wang-asplos17.pdf). Graspan is a system built to analyze large code bases, by transforming the code bases into directed graphs that describe the flow of data through variable bindings. The Graspan authors had to build a new system to be able to handle their queries (apparently [SociaLite](https://socialite-lang.github.io) bombed hard, rarely terminating, much like my browser when I click on [their "Quick Start" link](https://socialite-lang.github.io/pages/quick_start)).

The Graspan setting starts from some base "edges", extracted from a program's source. These edges have a few different semantic flavors, and you could imagine adding new ones if you have further interests. Examples include (quoted from their Section 2.2):
...
To approach this problem, we'll make the language a little simpler. The Graspan authors do something similar, but we are going to deviate a little bit to help with our approach. I'm just going to say where we arrive at, and leave it as a bit of an exercise to confirm that this language is similarly expressive.

Our query language will be a list of *productions*, where each production has the form:

    R0  ::==  R1 R2 .. Rk

There is just one relation named on the left side, and the relations on the right side can be either relations of transposed relations (`~`). The relations on the right side can occur multiple times, and can include the relation named on the left side. The same relation can be named in the left side of several distinct productions, and each production contributes to the elements of the left side relation.

This is a bit like the language the Graspan folks get to in their Section 3, but we don't allow empty productions (epsilon, in their framing). Instead, relations `R` that contain the empty production are replaced by ones `R+` that do not, and all productions that use such a relation are replaced by two productions: i. one using the positive relation and ii. one not using it. Ultimately, the property "contains the empty production" is a static property of a relation in a query, and one whose complexity we can optimize out.
...
struct Query {
    ...
    pub productions: Vec<Prouduction>,
}
```
...
Let's build a slightly simpler interface to `Variable`, an `EdgeVariable`. This variable can be created `from` an initial collection, and it can have other collections added in using `add_production`. In the `drop` code for the `EdgeVariable` we take the concatenation of all input collections and apply `distinct()` before binding the result as the definition of the wrapped `Variable`.
...
pub struct EdgeVariable<'a, G: Scope> where G::Timestamp : Lattice {
    variable: Option<Variable<'a, G, Edge, isize>>,             // An option, so that we can `take` it.
...
impl<'a, G: Scope> EdgeVariable<'a, G> where G::Timestamp : Lattice {
    ...
    pub fn from(source: &Collection<Child<'a, G, u64>, Edge>) -> Self {
        EdgeVariable {
            variable: Some(Variable::from(source.filter(|_| false))),
...
// When we drop the `EdgeVariable`, complete the recursive definition.
impl<'a, G: Scope> Drop for EdgeVariable<'a, G> where G::Timestamp : Lattice {
    fn drop(&mut self) {
        if let Some(variable) = self.variable.take() {
            variable.set(&self.current.distinct());
        }
    }
}
```

You may not entirely understand how `Variable` works internally, and this is fine because differential dataflow is mysterious in its ways, but the short version is that if we create an `EdgeVariable` and repeatedly call `add_production`, we will end up with a collection that in each round ensures that it contains all produced facts.
...
```rust
            ...
            for (name, (input, collection)) in input_map.drain() {
                ...
                let trace = edge_variable.variable.as_ref().unwrap().leave().arrange_by_self().trace;
            ...
            for production in self.productions.iter() {
                ...
                if production.relations.len() > 0 {
                    ...
                    let mut transposed = match production.relations[0] {
                        Relation::Forward(ref name) => variable_map[name].current.map(|(x,y)|(y,x)),
                        Relation::Reverse(ref name) => variable_map[name].current.clone(),
                    };
                    ...
                    for relation in production.relations[1..].iter() {
                        let to_join = match relation {
                            &Relation::Forward(ref name) => variable_map[name].current.clone(),
                            &Relation::Reverse(ref name) => variable_map[name].current.map(|(x,y)|(y,x)),
                        };
...
Here is a program that runs a standard Graspan query: the "dataflow" query which tracks null assignments. Ignore the lines about `render_in_opt` for the moment; they are the subject of the next section.
...
fn main() {
    ...
    timely::execute_from_args(std::env::args(), move |worker| {
        ...
        let mut relation_map = worker.dataflow::<(),_,_>(move |scope| {
            // static reachability query.
            let query = Query::build_from(vec!["nodes nodes edges".to_string(), "edges".to_string()]);
            if std::env::args().any(|x| x == "opt") {
                query.render_in_opt(scope)
            }
            else {
                query.render_in(scope)
            }
        });

        if index == 0 { println!("{:?}:\tDataflow assembled", timer.elapsed()); }

        // Extract input handles so we can start to work with them.
        let mut nodes = relation_map.remove("nodes").unwrap();
        let mut edges = relation_map.remove("edges").unwrap();

        let timer1 = timer.clone();
        let timer2 = timer.clone();

        // Build an additional dataflow to report the resulting sizes.
        worker.dataflow(|scope| {
            nodes.trace
                 .import(scope)
                 .as_collection(|&_kv,&()| ())
                 .consolidate()
                 .inspect(move |x| println!("{:?}\tnodes: {:?}", timer1.elapsed(), x.2));
            edges.trace
                 .import(scope)
                 .as_collection(|&_kv,&()| ())
                 .consolidate()
                 .inspect(move |x| println!("{:?}\tedges: {:?}", timer2.elapsed(), x.2));
        });

        // Read the input file, populate the input relations.
        let filename = std::env::args().nth(1).expect("Where is the filename?");
        let file = BufReader::new(File::open(filename).expect("Couldn't open the file."));
        for readline in file.lines() {
            let line = readline.ok().expect("read error");
            if !line.starts_with('#') && line.len() > 0 {
                let mut elts = line[..].split_whitespace();
                let src = elts.next().expect("No src found").parse().ok().expect("malformed src");
                if (src as usize) % peers == index {
                    let dst = elts.next().expect("No dst found").parse().ok().expect("malformed dst");
                    let val: &str = elts.next().expect("No val found");
                    match val {
                        "n" => { nodes.input.insert((src, dst)); },
                        "e" => { edges.input.insert((src, dst)); },
...
We can run this with one of the input datasets graciously provided by the Graspan team (thank you, Graspan team!), the `nodes` and `edges` relations for `httpd`.

    Echidnatron% cargo run --release --example graspan -- ~/Desktop/graspan/httpd_df.dms
    ...
    Rendering: Query {
        productions: [
            Production { left_hand: "nodes", relations: [Forward("nodes"), Forward("edges")] },
            Production { left_hand: "edges", relations: [] },
        ]
    }
...
We can do this with an optimized form of the `EdgeVariable` struct, which will maintain indexed forms of the forward and reverse relations:

```rust
pub struct EdgeVariableOpt<'a, G: Scope> where G::Timestamp : Lattice {
    variable: Option<Variable<'a, G, Edge, isize>>,         // An option, so that we can drop it.
    current: Collection<Child<'a, G, u64>, Edge, isize>,
    pub forward: Arrange<Child<'a, G, u64>, Node, Node, isize>,
    pub reverse: Arrange<Child<'a, G, u64>, Node, Node, isize>,
}
```
...
which uses the lower-level `join_core` method that allows you to supply pre-indexed arranged data, rather than un-indexed collections. Now we can re-use a variable multiple times without needing to maintain multiple copies in memory. For example, we could track all paths along five hops of edges:
...
It turns out there is another, very similar, bug. Let's zoom in on it:
...
That's the last line or two of the `scoped` call. Looks .. simple enough? What happens there?

Remember up at the top of the scope, we had a

```rust
    let mut variable_map = HashMap::new();
```

and that this map holds on to our `EdgeVariable` things, and that these clever structs have some code in their `drop` methods that connect bits of dataflow, and because it is a `HashMap` they `drop` methods GET CALLED IN A NON-DETERMINISTIC ORDER. Geez.

Let's fix that last line or two of code with some unmitigated brilliance:

```rust
        let mut var_map = variable_map.drain().collect::<Vec<_>>();
        var_map.sort_by(|x,y| x.0.cmp(&y.0));

        result_map
    })
```
Here we drain the map, sort the results, and then drop the result when we exit the scope, thereby ensuring that the `EdgeVariable` things are dropped in a consistent order. Maybe `EdgeVariable` wasn't such a smart idea after all, or maybe we just need to avoid using `HashMap`s so casually. There is a Rust crate for an [ordered hash map](https://crates.io/crates/ordermap) which maintains the insertion order, provides iteration in that order, and (presumably?) drops in the insertion order.
