...
There is only one edge out of `2`, to node `4`, which we can see. Also, our reachability query indicates that we can read both of these nodes from `2`, which non-trivially tells us that we can't reach any *other* nodes, yet.
...
This transaction wants to add `(5,0)`, which would be awesome, but it needs both `(0,1)` and `(0,2)` to be present in the graph. The first edge is, but the second edge is not, so `aborts` gets a new transaction id, `4`, and the writes are not commited. I secretly put an `inspect` as part of `aborts` so that we could see:
...
Let's do some actual transactions now. I have no clue what a good workload is here, and I'm a bit lazy. So I just made something up. We are going to send in 1,000 batches of `batch` transactions, each of which pulls a read requirement and a write location at random from a fixed set of size `batch`. This should make a non-trivial constant fraction of the transactions risk conflict, and force our craaaaazy transaction resolver to do some work (maybe).
