...
This has a pretty easy differential dataflow implementation:
...
tripdata
    ...
    .reduce(|key, input, output| {
        ...
        // Load up the key as the first columns.
        for col in key.iter() {
            result.push(col.0.clone());
        }
        // Insert aggregates in order.
        for agg_fn in agg_fns {
            result.push(agg_fn(input));
        }
...
To do this, we implement each `SUM` by tracking three aggregate quantities for each key:
1. The number of values,
2. The number of non-NULL values,
3. The aggregate value itself.

Not all of the aggregates need this complexity (`COUNT(*)` just needs the first number) but we use three just to keep the dataflow logic simple enough. In the future, we should reasonably fuse all of these aggregations together, as without distinct values there is no harm in doing so, and many share the same aggregate quantities.
