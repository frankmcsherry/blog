...
What a `Layer<T>` records are the various ways we can extend a tuple of values with some new value `T`, resulting in a tuple of one greater dimension.
We'll start with the empty tuple `()` in mind, followed by:
1. The first layer has a single list which are the distinct values we could start with.
1. The second layer has as many lists as the first has values, and each list contains the distinct values that can be appended to the corresponding tuple.
1. The next layer has as many lists as the second has values, and each lists contains the distinct values that can be appended to the corresponding tuple.
1. ...
