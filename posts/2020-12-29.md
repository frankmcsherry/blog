...
The logic generalizes to hours, days, etc., and can be used on the way back down if you round from `time2`.
...
There can be repetition if e.g. `time1` and `time2` are within the same day (or hour).

### Aggregating input each minute, hour, and day