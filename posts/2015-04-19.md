...
A `GraphRoot` is pretty simple, actually. It is the simplest implementor of the `GraphBuilder` trait, whose main role in life is to hand back references to a `Communicator` so that we can make channels.
