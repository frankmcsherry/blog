...
The first and immediate block of output is the "initial snapshot timestamp" progress message, which tells us the time the initial snapshot of the `SUBSCRIBE` will reflect.
The second block of output includes the snapshot, and as it requires spinning up a dataflow (`winning_bids` is a non-materialized view) also some additional changes that happen before we are able to report the snapshot.
The next block is live and reports a new update as it happens, from `38550` to `38551`.
The last block reports multiple seconds proceeding for which the count does not change.

Importantly, these blocks are each reports of the correct `COUNT(*)` output at each time, rather than "what happens to be available". 
