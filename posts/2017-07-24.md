...
I was recently reading an slide deck from Azul which makes an excellent point, one I hadn't clearly understood: when taking latency measurements, there is a huge difference between *closed-loop* (where the harness waits for responses before making new requests) and *open-loop* (where the harness does not wait). 
