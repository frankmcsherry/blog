...
For those of you familiar with `map` and `reduce`, the `group_by` function is basically like `reduce`. The operator acts on a stream of key-value pairs, groups incoming pairs by their key value, and applies the reduction function `red` to each non-empty pair of key and matching records.
