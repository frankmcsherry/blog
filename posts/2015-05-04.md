**Note**: [the abomonation library](https://github.com/frankmcsherry/abomonation) has improved in the time since this post. I've left the text unadjusted, but several things are now out of date.
...
I could choose to remove the `unsafe` label and put unsafe internally like so, declaring the method safe:
...
This not only exposes mutable slices, but permits any type at all. Imagine what would happen if `T` was `String`, for example: `String` owns its `[u8]` buffer and contains a pointer to it. You would be able to dereference arbitrary places in memory! Danger!! Danger!! The `unsafe` label stays on for this one.
...
Really, the whole interface is probably just messing with forces we don't understand.
...
Yeah, there is a disastrous mess of things here, and really Rust's memory safety isn't touching any of this with a ten foot lifetime bound. But let's talk about the possible issues.
...
2. The `&mut self` parameter to `exhume` will only be presented outwards as a `&self`. We can't let anyone mutate or own this data. Trying to add elements to `Vec` will trigger a re-allocation, which would attempt to release the memory. It is very important that all references to the data are just that, only references. And references with lifetimes bounded by that of the source `&[u8]`.
